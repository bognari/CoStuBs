...
//        out.println("idle == true -> CPU::halt();");
        CPU::halt();
        CPU::disableInterrupts();
        next = (Activity*)readylist.dequeue(); // gucken ob nun doch was drinnen ist.
...
void ActivityScheduler::checkSlice() {
  ...
  if (!this -> idle) { // gucken ob er einen laufene act hat
    Activity* curr = (Activity*) this -> active();
    curr -> tickUp(); // da tick = 0 ja auch ein tick ist soellte man ihn auch mitzaehlen

    if (!((curr -> getTick()) < (curr -> quantum()))) { // pruefen ob die zeit schon um ist
