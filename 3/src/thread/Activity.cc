...
Activity::Activity(void* tos, int time): Coroutine(tos), Schedulable(time), parent(0), state(BLOCKED), children()
 ...
 * aufgerufen werden.
 */
Activity::Activity(int time): Coroutine(), Schedulable(time), parent(0), state(BLOCKED), children() // arbeitet auf dem mainstack also ist er cheffe
{
  this -> state = READY;
...
void Activity::sleep()
{
  ...
  if (this -> isRunning()) {
    ...
    this -> state = BLOCKED;
    scheduler.remove(this); // soll ja auch net mehr in der readylist stehen wenn der blocked ist ~~
  }
}
