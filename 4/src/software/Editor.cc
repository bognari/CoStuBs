...
//    this -> yPosi = 23;
    ...
    if ((entfernung < 0) || (richtung == UP)) { // hoch scrollen, da es aber iwie net will, geh ich ganz nach oben und scroll runter bis x
      this -> screenTop = (MyNode*)&(this -> content);
      entfernung = this -> content.getDistance(this -> screenTop, this -> cursor);
      while (entfernung >= 80) {
        this -> screenTop = (this -> content.getItemFrom(this -> screenTop, 80) -> getPrev());
        entfernung = this -> content.getDistance(this -> screenTop, this -> cursor);
      }
    }

    while ((entfernung >= 2000) && (richtung == NORMAL)) { //nach unten scrollen
      this -> screenTop = (this -> content.getItemFrom(this -> screenTop, 80) -> getPrev());
      entfernung = this -> content.getDistance(this -> screenTop, this -> cursor);
    }
