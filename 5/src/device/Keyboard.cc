...
Keyboard::Keyboard() : Gate(KeyboardInterrupt), dataPort(DATA), ctrlPort(CTRL), mode(0), scanCode(0)
{
	this -> setLed(SCROLL_LOCK_LED,false);
	this -> setLed(NUM_LOCK_LED,false);
	this -> setLed(CAPS_LOCK_LED,false);
	this -> setRepeatRate(0,0);
...
void Keyboard::reboot ()
{
	...
	*(unsigned short*) 0x472 = 0x1234;
	...
	waitForWrite();
	ctrlPort.write (RESET_CODE);     // Reset ausloesen
}
...
bool Keyboard::prologue()
{
  if (ctrlPort.read() & AUX_BIT) {
    ...
    unsigned char code = (unsigned char)(dataPort.read());
    this -> key_codes.add(code);
    pic.ack(this -> getNumber());
    return (true);
  }
}
...
{ // hier sichern
  IntLock lock;
  while (!(this -> key_codes.isEmpty())) {
    this -> scanCode = this -> key_codes.get();
    lock.restore();
